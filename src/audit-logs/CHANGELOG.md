...
- Renamed PyPI package from `audit-logs-mcp` to `greenlake-audit-logs-mcp` for consistent HPE GreenLake branding
- Moved source code into `greenlake_audit_logs_mcp/` package folder to resolve namespace collisions when multiple MCP servers are installed in the same Python environment

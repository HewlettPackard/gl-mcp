...
from greenlake_audit_logs_mcp.server.mcp_server import MCPServer
...
class TestMCPServer:
    ...
    @patch("greenlake_audit_logs_mcp.server.mcp_server.get_http_client")
    ...
    async def test_initialize_method(self):
        ...
        with (
            patch("greenlake_audit_logs_mcp.server.mcp_server.get_http_client", return_value=mock_http_client),
            patch("greenlake_audit_logs_mcp.server.mcp_server.get_tool_classes", return_value=[]),
        ):
            server = MCPServer()
...
class TestApp:
    ...
    def test_app_module_structure(self):
        ...
        import greenlake_audit_logs_mcp.server.app as app_module

        assert hasattr(app_module, "main") or hasattr(app_module, "run") or hasattr(app_module, "start")

    @patch("greenlake_audit_logs_mcp.server.app.mcp")
...
class TestServerIntegration:
    ...
    def test_server_dependencies_importable(self):
        ...
        # Test app imports
        try:
            import greenlake_audit_logs_mcp.server.app as _app_mod

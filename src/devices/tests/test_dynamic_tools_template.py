...
from greenlake_devices_mcp.tools.implementations.list_endpoints import list_endpoints
from greenlake_devices_mcp.tools.implementations.get_endpoint_schema import get_endpoint_schema
from greenlake_devices_mcp.tools.implementations.invoke_dynamic_tool import invoke_dynamic_tool
...
class TestGetEndpointSchema:
    ...
    async def test_valid_endpoint_returns_schema(self):
        ...
        result = await get_endpoint_schema(endpoint_identifier="GET:/devices/v1/devices")

        assert len(result) == 1
        response = result[0]
        assert response["success"] is True
        assert response["endpoint_identifier"] == "GET:/devices/v1/devices"
        schema = response["schema"]
        assert schema["path"] == "/devices/v1/devices"
...
class TestInvokeDynamicTool:
    ...
    async def test_unsupported_method_rejected(self):
        ...
        result = await invoke_dynamic_tool(
            ctx,
            endpoint_identifier="OPTIONS:/devices/v1/devices",
        )
    ...
    async def test_validation_catches_missing_required_param(self):
        ...
        result = await invoke_dynamic_tool(
            ctx,
            endpoint_identifier="GET:/devices/v1/devices/{id}",

...
"""
HTTP client for reporting MCP server.
...
from greenlake_reporting_mcp.auth.token_manager import TokenManager
from greenlake_reporting_mcp._version import USER_AGENT


class ReportingHttpClient:
    """HTTP client for reporting API with authentication."""
...
def get_http_client() -> "ReportingHttpClient":
    """Get the global HTTP client instance (lazy initialization)."""
    global _http_client
    if _http_client is None:
        _http_client = ReportingHttpClient()
    return _http_client

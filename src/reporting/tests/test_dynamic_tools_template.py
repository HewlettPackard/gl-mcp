...
"""
Tests for dynamic tools in reporting MCP server.
...
class TestListEndpointsTool:
    ...
    def test_tool_properties(self, tool):
        ...
        assert "reporting" in tool.description
...
class TestGetEndpointSchemaTool:
    ...
    async def test_execute_valid_endpoint(self, tool):
        ...
        result = await tool.execute({"endpoint_identifier": "GET:/reporting/v1/report-exports-metadata"})
    ...
    async def test_execute_with_examples(self, tool):
        ...
        result = await tool.execute(
            {"endpoint_identifier": "GET:/reporting/v1/report-exports-metadata", "include_examples": True}
        )
...
class TestInvokeDynamicTool:
    ...
    async def test_execute_valid_get_request(self, tool, mock_http_client):
        ...
        with patch.object(tool, "http_client", mock_http_client):
            result = await tool.execute(
                {"endpoint_identifier": "GET:/reporting/v1/report-exports-metadata", "parameters": test_parameters}
            )
    ...
    async def test_execute_unsupported_method(self, tool):
        ...
        result = await tool.execute(
            {
                "endpoint_identifier": "POST:/reporting/v1/report-exports-metadata",  # Valid path but unsupported method
                "parameters": {},
            }
        )
    ...
    return [
        {
            ...
            "parameters": [
                {"name": "filter", "type": "str", "required": True, "location": "query"},
                {"name": "select", "type": "str", "required": True, "location": "query"},
                {"name": "sort", "type": "str", "required": True, "location": "query"},
...
def sample_tool_arguments():
    ...
    return {
        ...
        "get_endpoint_schema": {
            "endpoint_identifier": "GET:/reporting/v1/report-exports-metadata",

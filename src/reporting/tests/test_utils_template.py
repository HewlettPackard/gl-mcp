...
"""
Tests for utility components in reporting MCP server.
...
class TestReportingHttpClient:
    """Test cases for ReportingHttpClient."""
    ...
    async def test_put_request_success(self, http_client, mock_token_manager):
        ...
        update_data = {"name": "Updated Resource for reporting", "status": "modified"}
    ...
    async def test_request_with_query_params(self, http_client, mock_token_manager):
        ...
        query_params = {"filter": "active", "limit": 10, "service": "reporting"}
...
class TestHttpClientIntegration:
    ...
    def integration_token_manager(self):
        ...
        mock_manager = Mock()
        mock_manager.get_auth_headers.return_value = {
            "Authorization": "Bearer integration-token-reporting",
            "Content-Type": "application/json",
        }
        return mock_manager
    ...
    async def test_full_request_cycle(self, integration_token_manager):
        ...
        with _maybe_patch_token_manager(integration_token_manager):
            http_client = ReportingHttpClient()

            with patch.object(http_client.client, "get") as mock_get:
                mock_response = Mock()
                mock_response.status_code = 200
                mock_response.json.return_value = {"service": "reporting", "status": "healthy", "version": "1.0.0"}
                mock_response.raise_for_status.return_value = None
                mock_get.return_value = mock_response
                ...
                # Verify result
                assert result["service"] == "reporting"
                assert result["status"] == "healthy"
                # Verify authentication was included
                call_args = mock_get.call_args
                headers = call_args.kwargs.get("headers", {})
                assert "integration-token-reporting" in headers.get("Authorization", "")
...
class TestHttpClientErrorHandling:
    ...
    async def test_large_response_handling(self, error_test_client):
        ...
        large_data = {"items": [{"id": i, "name": f"Item {i} for reporting", "data": "x" * 1000} for i in range(100)]}
        ...
        with patch.object(error_test_client.client, "get") as mock_get:
            ...
            assert result["items"][0]["name"] == "Item 0 for reporting"
...
def sample_api_response():
    """Sample API response data for reporting."""
    return {
        "id": "sample-id-reporting",
...
def sample_error_response():
    ...
    return {
        "error": {
            ...
            "message": "Resource not found in reporting",
            "details": {"service": "reporting", "timestamp": "2024-01-01T00:00:00Z"},
        }
    }

...
def get_tool_classes(mode: str | None = None) -> list:
    ...
    try:
        if mode == "dynamic":
            ...
            import tools.implementations.get_reporting_v1_report_exports_metadata  # noqa: F401 (triggers @mcp.tool registration)
            import tools.implementations.getreportingstatuses  # noqa: F401 (triggers @mcp.tool registration)
            import tools.implementations.getreportingstatusbyid  # noqa: F401 (triggers @mcp.tool registration)

            logger.info("Static mode: 3 endpoint tools registered")

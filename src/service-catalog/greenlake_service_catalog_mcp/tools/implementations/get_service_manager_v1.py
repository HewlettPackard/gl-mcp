...
"""
get_service_manager_v1 tool for service-catalog MCP server.

Implements: GET /service-catalog/v1/service-managers/{id}
"""
...
async def get_service_manager_v1(  # noqa: E501
    ctx: Context,
    id: Annotated[
        str,
        Field(description="Service manager ID"),
    ] = ...,
) -> list[dict[str, Any]]:
    ...
    url = "/service-catalog/v1/service-managers/{id}"
    ...
    try:
        ...
        logger.error(f"Validation error in get_service_manager_v1: {exc}")
        return [{"success": False, "error": "validation_error", "message": str(exc)}]

    except Exception as exc:
        logger.error(f"Error in get_service_manager_v1: {exc}", exc_info=True)

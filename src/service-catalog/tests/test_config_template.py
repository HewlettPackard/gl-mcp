...
"""
Tests for config components in service-catalog MCP server.
...
from greenlake_service_catalog_mcp.config.settings import settings, Settings
from greenlake_service_catalog_mcp.config.logging import setup_logging, get_logger
...
class TestSettings:
    ...
    def test_settings_service_specific_config(self):
        """Test service-specific configuration for service-catalog."""
...
class TestLogging:
    ...
    def test_setup_logging_default(self):
        ...
        logger = get_logger("test_service_catalog")
    ...
    def test_setup_logging_with_level(self):
        ...
        for level in levels:
            ...
            logger = get_logger("test_level_service_catalog")
    ...
    def test_get_logger_names(self):
        """Test logger naming for service-catalog."""
        # Test various logger names
        test_names = [
            "service_catalog.auth",
            "service_catalog.server",
            "service_catalog.tools",
            "service_catalog.http_client",
        ]
    ...
    def test_logging_output_format(self):
        ...
        setup_logging()
        logger = get_logger("format_test_service_catalog")

        # Test that logger can handle different log levels
        logger.info("Test info message for service-catalog")
        logger.warning("Test warning message for service-catalog")
        logger.error("Test error message for service-catalog")
    ...
    def test_structured_logging(self):
        ...
        setup_logging()
        logger = get_logger("structured_test_service_catalog")
    ...
    def test_logging_with_environment_config(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            setup_logging()
            logger = get_logger("env_test_service_catalog")

            # Should create logger successfully with env config
            assert logger is not None

    def test_logger_hierarchy(self):
        """Test logger hierarchy for service-catalog components."""
        setup_logging()

        # Create loggers with hierarchical names
        root_logger = get_logger("service_catalog")
        auth_logger = get_logger("service_catalog.auth")
        server_logger = get_logger("service_catalog.server")
...
class TestConfigIntegration:
    ...
    def test_settings_and_logging_integration(self):
        ...
        logger = get_logger("integration_test_service_catalog")
    ...
    def test_config_validation_with_dependencies(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            # Validate required fields for service-catalog
...
class TestConfigSecurity:
    ...
    def test_sensitive_data_not_logged(self):
        ...
        setup_logging()
        logger = get_logger("security_test_service_catalog")

        # Create settings with sensitive data
        test_env = {
            "GREENLAKE_CLIENT_ID": "test-client-id-security",
            "GREENLAKE_CLIENT_SECRET": "super-secret-value-service-catalog",
            "GREENLAKE_WORKSPACE_ID": "test-workspace-id-security",
            "TOKEN_ISSUER": "https://secure.service-catalog.com/oauth2/token",
        }
...
def mock_config_file():
    ...
    config_content = """
    CLIENT_ID=mock-client-id-service-catalog
    CLIENT_SECRET=mock-client-secret-service-catalog
    TOKEN_ISSUER=https://mock.service-catalog.com/oauth2/token
...
class TestConfigEdgeCases:
    ...
    def test_unicode_config_values(self):
        ...
        unicode_env = {
            "CLIENT_ID": "test-中文-service-catalog",
            "TOKEN_ISSUER": "https://тест.service-catalog.com/oauth2/token",
        }

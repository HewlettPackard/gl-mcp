...
"""
Tests for utility components in service-catalog MCP server.
...
from greenlake_service_catalog_mcp.utils.http_client import ServiceCatalogHttpClient, get_http_client
...
def _maybe_patch_token_manager(manager_mock):
    ...
    with patch("greenlake_service_catalog_mcp.utils.http_client.TokenManager", return_value=manager_mock):
        yield


class TestServiceCatalogHttpClient:
    """Test cases for ServiceCatalogHttpClient."""
    ...
    def http_client(self, mock_token_manager):
        ...
        with _maybe_patch_token_manager(mock_token_manager):
            return ServiceCatalogHttpClient()
    ...
    async def test_put_request_success(self, http_client, mock_token_manager):
        ...
        update_data = {"name": "Updated Resource for service-catalog", "status": "modified"}
    ...
    async def test_request_with_query_params(self, http_client, mock_token_manager):
        ...
        query_params = {"filter": "active", "limit": 10, "service": "service-catalog"}
...
class TestHttpClientFactory:
    ...
    def test_get_http_client(self):
        ...
        with patch("greenlake_service_catalog_mcp.utils.http_client.ServiceCatalogHttpClient") as mock_client_class:
            mock_client = Mock()
    ...
    def test_get_http_client_multiple_calls(self):
        ...
        import greenlake_service_catalog_mcp.utils.http_client as _http_mod

        _http_mod._http_client = None
        # Mock TokenManager to avoid real authentication
        with patch("greenlake_service_catalog_mcp.utils.http_client.TokenManager") as mock_token_manager_class:
...
class TestHttpClientIntegration:
    ...
    def integration_token_manager(self):
        ...
        mock_manager = Mock()
        mock_manager.get_auth_headers.return_value = {
            "Authorization": "Bearer integration-token-service-catalog",
            "Content-Type": "application/json",
        }
        return mock_manager
    ...
    async def test_full_request_cycle(self, integration_token_manager):
        ...
        with _maybe_patch_token_manager(integration_token_manager):
            http_client = ServiceCatalogHttpClient()
            ...
            with patch.object(http_client.client, "get") as mock_get:
                ...
                # Verify result
                assert result["service"] == "service-catalog"
                assert result["status"] == "healthy"
                # Verify authentication was included
                call_args = mock_get.call_args
                headers = call_args.kwargs.get("headers", {})
                assert "integration-token-service-catalog" in headers.get("Authorization", "")
...
class TestHttpClientErrorHandling:
    ...
    async def test_large_response_handling(self, error_test_client):
        ...
        large_data = {
            "items": [{"id": i, "name": f"Item {i} for service-catalog", "data": "x" * 1000} for i in range(100)]
        }
        ...
        with patch.object(error_test_client.client, "get") as mock_get:
            ...
            assert result["items"][0]["name"] == "Item 0 for service-catalog"
...
def sample_api_response():
    """Sample API response data for service-catalog."""
    return {
        "id": "sample-id-service-catalog",
        "name": "Sample Resource",
        "type": "ServiceCatalogResource",
        "status": "active",
        "metadata": {
...
def sample_error_response():
    ...
    return {
        "error": {
            ...
            "message": "Resource not found in service-catalog",
            "details": {"service": "service-catalog", "timestamp": "2024-01-01T00:00:00Z"},
        }
    }

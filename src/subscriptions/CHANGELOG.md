...
- Renamed PyPI package from `subscriptions-mcp` to `greenlake-subscriptions-mcp` for consistent HPE GreenLake branding
- Moved source code into `greenlake_subscriptions_mcp/` package folder to resolve namespace collisions when multiple MCP servers are installed in the same Python environment

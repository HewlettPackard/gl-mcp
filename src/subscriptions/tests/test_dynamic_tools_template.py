...
from greenlake_subscriptions_mcp.tools.implementations.list_endpoints import list_endpoints
from greenlake_subscriptions_mcp.tools.implementations.get_endpoint_schema import get_endpoint_schema
from greenlake_subscriptions_mcp.tools.implementations.invoke_dynamic_tool import invoke_dynamic_tool
...
class TestGetEndpointSchema:
    ...
    async def test_valid_endpoint_returns_schema(self):
        ...
        result = await get_endpoint_schema(endpoint_identifier="GET:/subscriptions/v1/subscriptions")

        assert len(result) == 1
        response = result[0]
        assert response["success"] is True
        assert response["endpoint_identifier"] == "GET:/subscriptions/v1/subscriptions"
        schema = response["schema"]
        assert schema["path"] == "/subscriptions/v1/subscriptions"
...
class TestInvokeDynamicTool:
    ...
    async def test_unsupported_method_rejected(self):
        ...
        result = await invoke_dynamic_tool(
            ctx,
            endpoint_identifier="OPTIONS:/subscriptions/v1/subscriptions",
        )

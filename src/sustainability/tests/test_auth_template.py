...
"""
Tests for authentication components in Sustainability_Insight_Center MCP server.
...
class TestTokenInfo:
    ...
    def test_token_info_creation(self):
        ...
        token_info = TokenInfo(
            token="test-token-sustainability-insight-center",
            expires_at=time.time() + 3600,
        )

        assert token_info.token == "test-token-sustainability-insight-center"
...
class TestOAuth2Provider:
    ...
    def oauth2_provider(self):
        ...
        return OAuth2Provider(
            ...
            token_url="https://sustainability-insight-center.example.com/oauth2/token",
    ...
    def mock_token_response(self):
        ...
        return {
            "access_token": "test-access-token-sustainability-insight-center",
            "token_type": "Bearer",
            "expires_in": 3600,
            "scope": "read write",
        }
    ...
    def test_oauth2_provider_initialization(self, oauth2_provider):
        """Test OAuth2Provider initialization for Sustainability_Insight_Center."""
        assert oauth2_provider.client_id == "test-client-id"
        assert oauth2_provider.client_secret == "test-client-secret"
        assert "sustainability-insight-center" in oauth2_provider.token_url
    ...
    def test_get_token_success(self, mock_client_class, mock_token_response):
        """Test successful token acquisition for Sustainability_Insight_Center."""
        ...
        assert token.access_token == "test-access-token-sustainability-insight-center"
        ...
        mock_client_instance.post.assert_called_once_with(
            "https://sustainability-insight-center.example.com/oauth2/token",
            headers={
    ...
    def test_get_token_failure(self, mock_client_class):
        """Test failed token acquisition for Sustainability_Insight_Center."""
        ...
        mock_response.raise_for_status = Mock(
            side_effect=Exception(
                "Token request failed: HTTP 400: Bad Request: Invalid client credentials"
            )
        )
        mock_client_instance.post.return_value = mock_response
        mock_client_instance.__enter__ = Mock(return_value=mock_client_instance)
    ...
    def test_oauth2_token_response_expiry_calculation(self):
        ...
        if token.expires_at:
            assert (
                abs(token.expires_at - expected_expiry) < 2
            )  # Allow 2 second tolerance

    def test_validate_token(self, oauth2_provider):
        """Test token validation for Sustainability_Insight_Center."""
        # Test valid token
...
class TestTokenManager:
    ...
    def mock_settings(self):
        """Mock settings for testing Sustainability_Insight_Center."""
        settings = Mock()
    ...
    def valid_token(self):
        ...
        return OAuth2TokenResponse(
            access_token="valid-token-sustainability-insight-center",
            token_type="Bearer",
            expires_in=3600,
            scope="read write",
        )
    ...
    def expired_token(self):
        ...
        return OAuth2TokenResponse(
            access_token="expired-token-sustainability-insight-center",
            token_type="Bearer",
    ...
    def test_token_manager_initialization(self):
        """Test TokenManager initialization for Sustainability_Insight_Center with lazy token loading."""
    ...
    def test_get_valid_token_when_none_cached(
        self, mock_provider_class, mock_settings, valid_token
    ):
        """Test lazy token generation when none is cached for Sustainability_Insight_Center."""
        # Setup mock provider
        mock_provider = Mock()
    ...
    @patch("auth.token_manager.OAuth2Provider")
    def test_get_valid_token_when_valid_cached(
        self, mock_provider_class, mock_settings, valid_token
    ):
        """Test getting token when valid one is cached for Sustainability_Insight_Center."""
    ...
    @patch("auth.token_manager.OAuth2Provider")
    def test_get_valid_token_when_expired_cached(
        self, mock_provider_class, mock_settings, expired_token, valid_token
    ):
        """Test automatic token refresh when expired token is cached for Sustainability_Insight_Center."""
        # Setup mock provider
        mock_provider = Mock()
        mock_provider.get_token.return_value = valid_token
        mock_provider_class.return_value = mock_provider
    ...
    @patch("auth.token_manager.OAuth2Provider")
    def test_lazy_initialization_multiple_calls(
        self, mock_provider_class, mock_settings, valid_token
    ):
    ...
    @patch("auth.token_manager.OAuth2Provider")
    def test_initialization_with_initial_token(
        ...
        initial_token = "pre-provided-token-sustainability-insight-center"
        token_manager = TokenManager(
            settings=mock_settings, initial_token=initial_token
        )
    ...
    @patch("auth.token_manager.OAuth2Provider")
    def test_token_refresh_on_auth_error(
        self, mock_provider_class, mock_settings, valid_token
    ):
        ...
        refreshed_token = OAuth2TokenResponse(
            access_token="refreshed-token-sustainability-insight-center",
            token_type="Bearer",
            expires_in=3600,
            scope="read write",
        )
    ...
    def test_get_authorization_header(
        ...
        """Test getting authorization header with lazy initialization for Sustainability_Insight_Center."""
        # Setup mock provider
        mock_provider = Mock()
        mock_provider.get_token.return_value = valid_token
        mock_provider_class.return_value = mock_provider
...
class TestAuthIntegration:
    ...
    def mock_settings(self):
        ...
        settings.token_issuer = (
            "https://sustainability-insight-center.integration.com/oauth2/token"
        )
    ...
    def test_end_to_end_token_flow(self, mock_client_class, mock_settings):
        """Test complete lazy token acquisition and management flow for Sustainability_Insight_Center."""
        # Setup mock httpx.Client
        mock_client_instance = Mock()
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "access_token": "integration-test-token-sustainability-insight-center",
            "token_type": "Bearer",
            "expires_in": 3600,
        ...
        assert "Authorization" in auth_headers
        assert (
            "integration-test-token-sustainability-insight-center"
            in auth_headers["Authorization"]
        )
...
class TestAuthErrorHandling:
    ...
    def test_token_manager_handles_oauth_errors(self, mock_provider_class):
        ...
        mock_settings = Mock()
        mock_settings.client_id = (
            "production-id"  # Use non-test ID to bypass test detection
        )

...
"""
Tests for config components in Sustainability_Insight_Center MCP server.
...
class TestSettings:
    ...
    def test_settings_from_environment(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            assert (
                test_settings.greenlake_api_base_url
        ...
        with patch.dict(os.environ, {}, clear=True):
            try:
                ...
                assert any(
                    ...
                    for field in ["client_id", "client_secret", "workspace"]
                )
    ...
    def test_settings_service_specific_config(self):
        """Test service-specific configuration for Sustainability_Insight_Center."""
...
class TestLogging:
    ...
    def test_setup_logging_default(self):
        ...
        logger = get_logger("test_sustainability_insight_center")
    ...
    def test_setup_logging_with_level(self):
        ...
        for level in levels:
            ...
            logger = get_logger("test_level_sustainability_insight_center")
    ...
    def test_get_logger_names(self):
        """Test logger naming for Sustainability_Insight_Center."""
        # Test various logger names
        test_names = [
            "sustainability_insight_center.auth",
            "sustainability_insight_center.server",
            "sustainability_insight_center.tools",
            "sustainability_insight_center.http_client",
        ]
    ...
    def test_logging_output_format(self):
        ...
        setup_logging()
        logger = get_logger("format_test_sustainability_insight_center")

        # Test that logger can handle different log levels
        logger.info("Test info message for Sustainability_Insight_Center")
        logger.warning("Test warning message for Sustainability_Insight_Center")
        logger.error("Test error message for Sustainability_Insight_Center")
    ...
    def test_structured_logging(self):
        ...
        setup_logging()
        logger = get_logger("structured_test_sustainability_insight_center")

        # Test logging with structured data using loguru's bind()
        try:
            logger.bind(
                service="Sustainability_Insight_Center",
                component="test",
    ...
    def test_logging_with_environment_config(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            setup_logging()
            logger = get_logger("env_test_sustainability_insight_center")

            # Should create logger successfully with env config
            assert logger is not None

    def test_logger_hierarchy(self):
        """Test logger hierarchy for Sustainability_Insight_Center components."""
        setup_logging()

        # Create loggers with hierarchical names
        root_logger = get_logger("sustainability_insight_center")
        auth_logger = get_logger("sustainability_insight_center.auth")
        server_logger = get_logger("sustainability_insight_center.server")
...
class TestConfigIntegration:
    ...
    def test_settings_and_logging_integration(self):
        ...
        logger = get_logger("integration_test_sustainability_insight_center")
        ...
        try:
            logger.bind(
                ...
                api_base_url=getattr(
                    test_settings, "greenlake_api_base_url", "unknown"
                ),
    ...
    def test_config_validation_with_dependencies(self):
        ...
        with patch.dict(os.environ, test_env, clear=False):
            ...
            # Validate required fields for Sustainability_Insight_Center
...
class TestConfigSecurity:
    ...
    def test_sensitive_data_not_logged(self):
        ...
        setup_logging()
        logger = get_logger("security_test_sustainability_insight_center")

        # Create settings with sensitive data
        test_env = {
            "GREENLAKE_CLIENT_ID": "test-client-id-security",
            "GREENLAKE_CLIENT_SECRET": "super-secret-value-sustainability-insight-center",
            "GREENLAKE_WORKSPACE_ID": "test-workspace-id-security",
            "TOKEN_ISSUER": "https://secure.sustainability-insight-center.com/oauth2/token",
        }
            ...
            logger.info(
                ...
                extra={
                    ...
                    "has_client_secret": bool(
                        getattr(test_settings, "client_secret", None)
                    ),
                },
            )
...
def mock_config_file():
    ...
    config_content = """
    CLIENT_ID=mock-client-id-sustainability-insight-center
    CLIENT_SECRET=mock-client-secret-sustainability-insight-center
    TOKEN_ISSUER=https://mock.sustainability-insight-center.com/oauth2/token
...
class TestConfigEdgeCases:
    ...
    def test_empty_environment_variables(self):
        ...
        with patch.dict(os.environ, empty_env, clear=False):
            try:
                ...
                assert any(
                    field in str(e).lower() for field in ["client_id", "client_secret"]
                )
    ...
    def test_unicode_config_values(self):
        ...
        unicode_env = {
            "CLIENT_ID": "test-中文-Sustainability_Insight_Center",
            "TOKEN_ISSUER": "https://тест.sustainability-insight-center.com/oauth2/token",
        }

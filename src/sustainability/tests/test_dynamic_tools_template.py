...
"""
Tests for dynamic tools in Sustainability_Insight_Center MCP server.
...
class TestListEndpointsTool:
    ...
    def mock_http_client(self):
        """Mock HTTP client."""
        client = AsyncMock()
        client.base_url = "https://us-west.api.greenlake.hpe.com"
        return client
    ...
    def test_tool_properties(self, tool):
        ...
        assert "Sustainability_Insight_Center" in tool.description
...
class TestGetEndpointSchemaTool:
    ...
    async def test_execute_valid_endpoint(self, tool):
        ...
        result = await tool.execute({
            "endpoint_identifier": "GET:/sustainability-insight-ctr/v1beta1/ingests/{id}"
        })
    ...
    async def test_execute_invalid_endpoint(self, tool):
        ...
        result = await tool.execute({
            "endpoint_identifier": "GET:/nonexistent"
        })
...
class TestInvokeDynamicTool:
    ...
    async def test_execute_valid_get_request(self, tool, mock_http_client):
        ...
        if response["success"]:
            ...
            assert ("validation" in response.get("error", "").lower() or 
                   "parameter" in response.get("error", "").lower() or
                   "required" in response.get("error", "").lower())
    ...
    async def test_execute_unsupported_method(self, tool):
        ...
        result = await tool.execute({
            "endpoint_identifier": "POST:/sustainability-insight-ctr/v1beta1/ingests/{id}",  # Valid path but unsupported method
            "parameters": {}
        })
        ...
        assert ("read operations" in error_text.lower() or 
                "unsupported" in error_text.lower() or 
...
class TestDynamicToolsIntegration:
    ...
    async def test_discovery_flow(self, list_tool, schema_tool, execute_tool):
        ...
        execute_result = await execute_tool.execute({
            "endpoint_identifier": endpoint_identifier,
            "parameters": {}
        })

...
"""
Tests for utility components in Sustainability_Insight_Center MCP server.
...
from utils.http_client import SustainabilityInsightCenterHttpClient, get_http_client


class TestSustainabilityInsightCenterHttpClient:
    """Test cases for SustainabilityInsightCenterHttpClient."""
    ...
    def http_client(self, mock_token_manager):
        ...
        with patch('utils.http_client.TokenManager', return_value=mock_token_manager):
            return SustainabilityInsightCenterHttpClient()
    ...
    async def test_put_request_success(self, http_client, mock_token_manager):
        ...
        update_data = {
            "name": "Updated Resource for Sustainability_Insight_Center",
            "status": "modified"
        }
    ...
    async def test_request_with_query_params(self, http_client, mock_token_manager):
        ...
        query_params = {
            "filter": "active",
            "limit": 10,
            "service": "sustainability-insight-center"
        }
...
class TestHttpClientFactory:
    ...
    def test_get_http_client(self):
        ...
        with patch('utils.http_client.SustainabilityInsightCenterHttpClient') as mock_client_class:
            mock_client = Mock()
...
class TestHttpClientIntegration:
    ...
    def integration_token_manager(self):
        ...
        mock_manager = Mock()
        mock_manager.get_auth_headers.return_value = {
            "Authorization": "Bearer integration-token-sustainability-insight-center",
            "Content-Type": "application/json"
        }
        return mock_manager
    ...
    async def test_full_request_cycle(self, integration_token_manager):
        ...
        with patch('utils.http_client.TokenManager', return_value=integration_token_manager):
            http_client = SustainabilityInsightCenterHttpClient()
            ...
            with patch.object(http_client.client, 'get') as mock_get:
                ...
                # Verify result
                assert result["service"] == "Sustainability_Insight_Center"
                assert result["status"] == "healthy"
                
                # Verify authentication was included
                call_args = mock_get.call_args
                headers = call_args.kwargs.get("headers", {})
                assert "integration-token-sustainability-insight-center" in headers.get("Authorization", "")
...
class TestHttpClientErrorHandling:
    ...
    async def test_large_response_handling(self, error_test_client):
        ...
        large_data = {
            "items": [
                {"id": i, "name": f"Item {i} for Sustainability_Insight_Center", "data": "x" * 1000}
                for i in range(100)
            ]
        }
        ...
        with patch.object(error_test_client.client, 'get') as mock_get:
            ...
            assert result["items"][0]["name"] == "Item 0 for Sustainability_Insight_Center"
...
def sample_api_response():
    """Sample API response data for Sustainability_Insight_Center."""
    return {
        "id": "sample-id-sustainability-insight-center",
        "name": "Sample Resource", 
        "type": "SustainabilityInsightCenterResource",
        "status": "active",
        "metadata": {
...
def sample_error_response():
    ...
    return {
        "error": {
            ...
            "message": "Resource not found in Sustainability_Insight_Center",
            "details": {
                "service": "Sustainability_Insight_Center",

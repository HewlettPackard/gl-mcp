...
"""
Base tool class for Sustainability_Insight_Center MCP server.
...
class BaseTool(ABC):
    ...
    def __init__(self, http_client: "SustainabilityInsightCenterHttpClient"):
        """
        Initialize the tool.
    ...
    def coerce_string_to_int(self, value: Any, param_name: str) -> int:
        ...
        if isinstance(value, str):
            try:
                ...
                raise ValueError(
                    f"Parameter '{param_name}' must be an integer or integer string, got '{value}'"
                ) from e

        raise ValueError(
            f"Parameter '{param_name}' must be an integer or integer string, got {type(value).__name__}"
        )

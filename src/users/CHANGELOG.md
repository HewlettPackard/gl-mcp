...
- Renamed PyPI package from `users-mcp` to `greenlake-users-mcp` for consistent HPE GreenLake branding
- Moved source code into `greenlake_users_mcp/` package folder to resolve namespace collisions when multiple MCP servers are installed in the same Python environment

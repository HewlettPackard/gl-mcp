...
"""
FastMCP server instance and application lifespan for workspaces.
...
class AppContext:
    ...
    http_client: Any  # WorkspacesHttpClient
...
async def _lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    from greenlake_workspaces_mcp.utils.http_client import get_http_client  # noqa: PLC0415
    from greenlake_workspaces_mcp.config.logging import get_logger  # noqa: PLC0415

    log = get_logger(__name__)
    log.info("Initialising workspaces HTTP client...")

    http_client = get_http_client()
    try:
        log.info("workspaces MCP server ready")
        yield AppContext(http_client=http_client)
    finally:
        log.info("Shutting down workspaces HTTP client...")
...
#
#   @mcp.tool()
#   async def list_workspaces_items(ctx: Context, ...) -> ...:

...
from greenlake_workspaces_mcp.tools.implementations.list_endpoints import list_endpoints
from greenlake_workspaces_mcp.tools.implementations.get_endpoint_schema import get_endpoint_schema
from greenlake_workspaces_mcp.tools.implementations.invoke_dynamic_tool import invoke_dynamic_tool
...
class TestGetEndpointSchema:
    ...
    async def test_valid_endpoint_returns_schema(self):
        ...
        result = await get_endpoint_schema(endpoint_identifier="GET:/workspaces/v1/workspaces/{workspaceId}")

        assert len(result) == 1
        response = result[0]
        assert response["success"] is True
        assert response["endpoint_identifier"] == "GET:/workspaces/v1/workspaces/{workspaceId}"
        schema = response["schema"]
        assert schema["path"] == "/workspaces/v1/workspaces/{workspaceId}"
...
class TestInvokeDynamicTool:
    ...
    async def test_unsupported_method_rejected(self):
        ...
        result = await invoke_dynamic_tool(
            ctx,
            endpoint_identifier="OPTIONS:/workspaces/v1/workspaces/{workspaceId}",
        )
    ...
    async def test_validation_catches_missing_required_param(self):
        ...
        result = await invoke_dynamic_tool(
            ctx,
            endpoint_identifier="GET:/workspaces/v1/workspaces/{workspaceId}",
            parameters={},  # required "workspaceId" is intentionally omitted
        )
        assert len(result) == 1
        assert result[0]["success"] is False
        ...
        result = await invoke_dynamic_tool(
            ctx,
            endpoint_identifier="GET:/workspaces/v1/workspaces/{workspaceId}/contact",
            parameters={},  # required "workspaceId" is intentionally omitted
        )
        assert len(result) == 1
        assert result[0]["success"] is False
